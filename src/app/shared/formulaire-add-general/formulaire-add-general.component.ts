...
export class FormulaireAddGeneralComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    console.log(this.route.snapshot.pathFromRoot.toString())
    //console.log(this.route.snapshot.url[this.route.snapshot.url.length-2].toString())
    if (this.route.snapshot.pathFromRoot[this.route.snapshot.pathFromRoot.length -2].url.toString()==StagiairesPoes.STAGIAIRES){
